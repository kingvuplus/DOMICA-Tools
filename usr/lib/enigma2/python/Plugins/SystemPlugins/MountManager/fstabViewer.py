...
class fstabEditorScreen(Screen, ConfigListScreen, HelpableScreen):
	...
	def __init__(self, session, selectedEntry=None, addEntry=False, cloneEntry=False):
		...
		self["ColorActions"] = HelpableActionMap(self, "ColorActions",
		{
			...
			"blue": (self.removeEntry, _("Remove entry")),
		}, -1)
		...
		self.list.append(getConfigListEntry(_("options mount file system (defaults): "), self.options))
	...
	def saveEntry(self):
		...
		if self.passnum.value == "1" and self.mountpoint.value != "/":
			...
			return
		new_entry = [self.devicename.value, self.mountpoint.value, self.fstype.value, self.options.value, str(self.dumpfreq.value), self.passnum.value]
