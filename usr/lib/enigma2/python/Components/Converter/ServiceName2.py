...
class ServiceName2(Converter, object):
    ...
    def getIPTVProvider(self, refstr):
        ...
        if 'udp/239.1.1.' in refstr:
            return 'Real'
        if 'udp/238.0.0.' in refstr or 'udp/238.0.16.' in refstr or 'udp/233.191.12.' in refstr or 'udp/233.191.133.' in refstr:
            return 'Triolan'
        if '%3a1234' in refstr:
            return 'IPTV1'
        if '0:0:0:0:0:0:http%3a//' in refstr:
            return 'StarNet'
        if '4097' in refstr:
            return 'StreamTV'
        return ''
